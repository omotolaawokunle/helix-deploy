...
class BuildRunnerSSHConnection implements SSHConnectionInterface
{
    private SSH2 $ssh;
    ...
    {
        $this->ssh = $this->createSSHClient($this->runner->ip_address, (int) $this->runner->ssh_port);
        ...
        if (! $this->ssh->login($this->runner->ssh_user, $key)) {
            throw new SSHConnectionException(sprintf(
                'Unable to authenticate SSH session for build runner [%s] user [%s].',
                (string) $this->runner->getKey(),
                (string) $this->runner->ssh_user,
            ));
        }
    ...
    private function verifyFingerprint(): void
    {
        ...
        if ($hostKey === false || $hostKey === '') {
            throw new SSHConnectionException('Unable to retrieve build runner host public key for fingerprint verification.');
        }

        $receivedFingerprint = hash('sha256', $hostKey);
        $storedFingerprint = $this->runner->fingerprint;

        if ($storedFingerprint === null) {
            $this->runner->update(['fingerprint' => $receivedFingerprint]);
        ...
        if (! hash_equals((string) $storedFingerprint, $receivedFingerprint)) {
            throw new BuildRunnerSSHFingerprintMismatchException(
                runner: $this->runner,
    ...
    private function writeFingerprintStoredAudit(string $fingerprint): void
    {
        AuditLog::query()->create([
            'organization_id' => (string) $this->runner->organization_id,
            'actor_id' => null,
            'operation' => 'build_runner.fingerprint_stored',
            'resource_type' => BuildRunner::class,
            'resource_id' => (string) $this->runner->getKey(),
